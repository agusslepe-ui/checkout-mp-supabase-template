...
## T-022 — Cierre del núcleo productivo
...
- HOME Classic conserva selección directa. AGENCY Classic conserva consulta de sucursales e informa únicamente `agencyCode` al seleccionar una.
...
- `Dockerfile` continúa ejecutando exclusivamente la web mediante `npm start` y exponiendo el puerto HTTP.
- `Dockerfile.worker` usa la misma imagen, dependencias y código, no expone puerto y ejecuta `npm run shipping:worker`.
- EasyPanel mantiene web y worker como servicios separados. `SHIPPING_IMPORT_WORKER_ENABLED=true` está configurada únicamente en el servicio worker.
- Despliegue validado mediante operación sostenida, outcomes `idle` y procesamiento automático real de una nueva order.
...
**Estado: DESPLEGADA / VALIDADA EN PRODUCCIÓN - MANUAL ONE-SHOT** (2026-09-18).
...
**Estado:** WORKER DURABLE IMPLEMENTADO / DESPLEGADO / CONSUMIDO POR T-022.6-B (2026-09-19).
...
- Paid+queue legacy-safe fue agregado por la migración 010 y conectado al webhook; confirma financieramente incluso cuando una order legacy no tiene snapshot.
- Claim activo con `FOR UPDATE SKIP LOCKED`, sólo para orders pagadas y trabajos `queued` o `retryable` cuyo horario ya venció. Incrementa `attempt_count` y asigna lease. `unknown` nunca es elegible automáticamente.
...
- Visitar `/success` manualmente también limpia carrito/attempt. Riesgo UX aceptado temporalmente para evitar complejidad o autoridad falsa en frontend; una protección futura mediante flag de `sessionStorage` queda como mejora no bloqueante.
...
- La implementación fue auditada por Grok, tuvo commit y push, fue desplegada en EasyPanel y validada manualmente en navegador real. `/success` carga con el diseño y textos esperados, “Volver al inicio” funciona, el carrito queda vacío y el checkout attempt eliminado.
...
- **T-017.4 — Ejecución, QA y cierre:** COMPLETADA / VALIDADA EN PRODUCCIÓN. Las migraciones 007 y 008 están aplicadas; frontend/backend T-017 están desplegados y el QA real cubrió idempotencia y READY + order `paid`.
...
- **Hardening READY + order `paid`: DESPLEGADO / VALIDADO EN PRODUCCIÓN.** Un retry real coincidente respondió 409 `checkout_attempt_already_paid` con mensaje público controlado, sin reutilizar URL, recotizar, crear recursos, llamar Mercado Pago ni mutar attempt/order. No redirigió a Mercado Pago.
- **Evidencia final:** `max(order.id)` permaneció igual; `checkout_attempt.state` permaneció `ready`; la order permaneció `paid`. El QA correcto reconstruyó la intención desde `order_items`; un intento previo con columnas legacy produjo el mismatch esperado y cero efectos secundarios.
- **Verificación local del hardening:** 380/380 tests, 9 suites, 0 fallos; incluye READY+pending, READY+paid sin efectos secundarios y frontend sin limpieza del carrito.
...
- UX post-pago + cleanup COMPLETADOS / DESPLEGADOS / VALIDADOS EN PRODUCCIÓN.

### PENDIENTE

- Perfiles físicos reales de los paquetes y repetición de QA.
- MiCorreo `POST /shipping/import` post-pago.
- Catálogo/stock/imágenes/descripciones dinámicos.
- Hardening comercial: precio definitivo, rotación de credenciales expuestas, auditoría npm y etapas posteriores de dominio/frontend/SEO.
...
Implementado, corregido, auditado, aprobado y enviado a `main`. En el cierre del Paso 1 no se implementaba todavía el Paso 2, Correo Argentino, idempotencia durable, cambios al webhook ni migraciones nuevas. El Paso 2 quedó COMPLETADO después, el 2026-09-12.

Hechos verificados:

1. `src/catalog.js` usa `TEMPORARY_MAX_QUANTITY = 4` en los cuatro SKUs. Es un techo temporal. **No representa stock real.** El stock real será una evolución futura.
2. Existe el dominio autoritativo del carrito en `src/cart.js`.
3. Existe `POST /carrito/resumen`. No persiste datos. No llama a Supabase, Mercado Pago ni logística. No crea pedidos ni preferencias.
4. Se validan como máximo 50 entradas originales antes de agrupar. Los SKUs duplicados se agrupan. Después de agrupar se valida la cantidad acumulada contra `maxQuantity`.
5. Los precios, moneda e importes enviados por el navegador no son autoritativos. El backend calcula utilizando su catálogo.
6. `/cotizar-envio` continúa limitado a `quantity: 1`. La cotización de envío sigue en una sola unidad hasta implementar logística multítem correctamente.
7. Carrito inválido → HTTP 400 `{ "error": "Carrito inválido" }`.
8. Suite completa: **129/129** tests pasando. `git diff --check` pasó.
9. El código del Paso 1 ya fue commiteado y enviado a `main` en GitHub.
10. DEC-022 / T-017 de idempotencia durable permanecen fuera de T-016.
11. Las vulnerabilidades npm (2 moderate + 2 high) informadas durante `npm ci` no se remedian aquí. No se ejecutó `npm audit fix`. Deben auditarse en una tarea separada; no forman parte de T-016 Paso 1.

#### Paso 2 — Checkout multítem y Mercado Pago

**Estado:** COMPLETADO (2026-09-12)

Implementado por Codex, auditado por Grok (APROBADO CON OBSERVACIONES; las observaciones eran únicamente documentales) y aprobado por el usuario el 2026-09-12. El estado COMPLETADO rige desde esa aprobación, no desde la implementación local.

Hechos verificados:

1. `POST /crear-preferencia` acepta el contrato multítem `{ items, customer, delivery }`.
2. Conserva compatibilidad legacy con `{ sku, quantity, customer, delivery }` mapeado a un ítem.
3. Rechaza requests que mezclen ambos formatos con HTTP 400 `{ "error": "Carrito inválido" }`.
4. El contrato legacy sin `customer`/`delivery` sigue rechazándose con los mensajes previos.
5. Cliente y entrega se validan con `parseCheckoutInput` antes de Supabase o Mercado Pago.
6. `p_items` y `preference.items` salen del mismo cálculo autoritativo en `src/cart.js`. Una orden `pending`, N `order_items`, una preferencia, N ítems de Mercado Pago, un único `external_reference` exactamente el devuelto por la RPC.
7. El backend calcula precios, cantidades agrupadas y total en centavos desde el catálogo. El navegador no es fuente de importes. El envío no se incluye en el total.
8. Si la RPC falla, no se crea preferencia. Si Mercado Pago falla, error genérico.
9. Webhook, HMAC, `markOrderAsPaid` e `importesCoinciden` intactos. Sin migraciones nuevas. Sin frontend/`localStorage`. Sin DEC-022/T-017. Sin stock real.
10. Suite completa: **158/158** tests pasando (29 nuevos del Paso 2). `git diff --check` pasó. Sin lectura de `.env` ni llamadas reales.
...
**Estado:** COMPLETADA / VALIDADA EN PRODUCCIÓN (2026-09-17)
**Prioridad:** P2
...
- T-017.4 ejecución, QA y cierre — COMPLETADA / VALIDADA EN PRODUCCIÓN; migraciones, deploy, QA de idempotencia y READY + order `paid` completados.
...
7. **Idempotency QA:** mismo intento/retry reutilizó attempt, orden y preferencia; nueva intención creó entidades nuevas; doble clic/retry normal no duplicó la orden; READY + order `paid` devolvió 409 controlado sin efectos secundarios.
...
Observaciones históricas no bloqueantes: posible ventana breve de schema cache tras `NOTIFY pgrst`; se respetó el orden 007 antes de Node T-017; los tests SQL eran mayormente estáticos. El QA real cubrió reutilización, cambio de intención, doble clic/retry y order ya `paid` con attempt READY. La limpieza segura posterior al retorno queda fuera de T-017.

#### Resultado esperado del cutover T-017.4

El objetivo completo quedó validado en producción: un reintento o doble envío normal del mismo intento lógico no generó un segundo pedido ni una segunda preferencia, y un READY asociado a una order ya `paid` fue rechazado con 409 sin mutaciones ni recursos nuevos. T-017 queda COMPLETADA / VALIDADA EN PRODUCCIÓN.
