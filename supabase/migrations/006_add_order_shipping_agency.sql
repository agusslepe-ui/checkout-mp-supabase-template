-- Migration: add an authoritative MiCorreo agency snapshot to orders.
-- Review and apply manually only after explicit authorization.
-- Historical and home-delivery orders keep all agency columns null.

begin;

alter table public.orders
  add column shipping_agency_code text,
  add column shipping_agency_name text,
  add column shipping_agency_street_name text,
  add column shipping_agency_street_number text,
  add column shipping_agency_locality text,
  add column shipping_agency_postal_code text;

alter table public.orders
  drop constraint orders_shipping_option_id_check,
  drop constraint orders_shipping_delivery_type_check,
  drop constraint orders_shipping_option_coherence_check;
...
alter table public.orders
  ...
  add constraint orders_shipping_delivery_type_check
    check (
      ...
      or shipping_delivery_type in ('home', 'agency')
    ),
  ...
  add constraint orders_shipping_agency_cohesion_check
    check (
      case
        when shipping_delivery_type is null then num_nonnulls(
        ...
        when shipping_delivery_type = 'home' then num_nonnulls(
          shipping_agency_code,
          shipping_agency_name,
          shipping_agency_street_name,
          shipping_agency_street_number,
        ...
        when shipping_delivery_type = 'agency' then
          num_nonnulls(
            shipping_agency_code,
            shipping_agency_name,
            shipping_agency_street_name,
          ...
          and btrim(shipping_agency_code) <> ''
          and btrim(shipping_agency_name) <> ''
          and btrim(shipping_agency_street_name) <> ''
          and btrim(shipping_agency_street_number) <> ''
          and btrim(shipping_agency_locality) <> ''
          and btrim(shipping_agency_postal_code) <> ''
        else false
      end
    );

comment on column public.orders.shipping_agency_code is
  'Authoritative MiCorreo agency identity. Future shipping/import must send this value as shipping.agency.';
comment on column public.orders.shipping_agency_name is
  'MiCorreo agency display name captured when checkout is created.';
comment on column public.orders.shipping_agency_street_name is
  'MiCorreo agency street name captured when checkout is created.';
comment on column public.orders.shipping_agency_street_number is
  'MiCorreo agency street number captured when checkout is created.';
comment on column public.orders.shipping_agency_locality is
  'MiCorreo agency locality captured when checkout is created.';
comment on column public.orders.shipping_agency_postal_code is
  'MiCorreo agency postal code captured when checkout is created.';

-- Remove the T-020 signature before creating the agency-aware RPC.
...
comment on function public.create_pending_order_with_items(
  ...
  'Atomically creates one pending order, validated product items, shipping total, and optional authoritative MiCorreo agency snapshot. Backend only.';
